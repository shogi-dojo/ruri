...
class CLITest < Minitest::Test
  ...
  def test_successful_compile_exits_zero_and_writes_output
    ...
    assert_empty stderr
    # The CLI runs with chdir: @tmpdir, so its header names the source
    # relative to that directory; compare the generated forms and the
    # header separately.
    emitted = File.read(output, encoding: "UTF-8")
    expected = Ruri.compile(HELLO_SOURCE, path: input)
    assert_includes emitted, ";;; Generated by ruri #{Ruri::VERSION} from hello.ruri. DO NOT EDIT."
    assert_equal expected.split("\n\n", 2).last, emitted.split("\n\n", 2).last
    assert_empty stdout
  end
  ...
  def test_header_path_is_independent_of_invocation_style
    # The generated header must be reproducible: compiling the same file
    # by relative and by absolute path has to produce identical output,
    # otherwise checked-in .el files drift depending on how they were
    # compiled. run_cli already runs with chdir: @tmpdir.
    write_input("command :sample do\n  interactive\n  insert(\"hi\")\nend\n",
                name: "sample.ruri")
    absolute_source = File.join(@tmpdir, "sample.ruri")

    _, _, relative_status = run_cli("compile", "sample.ruri", "--output", "relative.el")
    _, _, absolute_status = run_cli("compile", absolute_source, "--output", "absolute.el")
    assert_predicate relative_status, :success?
    assert_predicate absolute_status, :success?

    relative_output = File.read(File.join(@tmpdir, "relative.el"))
    absolute_output = File.read(File.join(@tmpdir, "absolute.el"))
    assert_equal relative_output, absolute_output
    assert_includes relative_output, "from sample.ruri."
    refute_includes relative_output, @tmpdir
  end
